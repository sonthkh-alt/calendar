...
export default function AdminLeaders({ leaders, bans, onChanged }) {
  ...
  const [draft, setDraft] = useState({ full_name: '', position: '', leader_type: 'ban', ban_id: '', sort_order: 99 });
  ...
  const addNew = async () => {
    ...
    await upsertLeader({
      ...
      sort_order: Number(draft.sort_order) || 99, active: true,
    });
    setBusy(null); setAdding(false);
    setDraft({ full_name: '', position: '', leader_type: 'ban', ban_id: '', sort_order: 99 });
    onChanged?.();
  };
