...
3. Tiếp tục dán `supabase/seed.sql` → **Run** (tạo 4 Ban, lãnh đạo, 4 xe, lịch mẫu).
4. Vào **Settings → API**: chép `Project URL` và `anon public key`.

> ⚠️ **Quy tắc về sau:** khi hệ thống nâng cấp cấu trúc, CHỈ chạy lại `schema.sql`
> (an toàn, không mất dữ liệu). **KHÔNG chạy lại `seed.sql`** — file này nạp dữ liệu
> mẫu và chỉ dành cho lần khởi tạo đầu; nó cũng tự chặn nếu đã có dữ liệu.
