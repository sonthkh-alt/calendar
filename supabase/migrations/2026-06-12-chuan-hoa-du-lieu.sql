...
-- (ĐÃ BỎ) Trước đây có bước "đồng bộ full_name dòng Ban = tên Ban" cho mô hình
-- cũ (mỗi Ban là 1 dòng đơn vị). Nay mỗi Ban gồm NHIỀU thành viên đích danh
-- (leader_type='ban' + ban_id) -> bước đó GHI ĐÈ họ tên thành viên thành tên Ban
-- (vd "Hoàng Anh Tuấn" -> "Ban Kinh tế Ngân sách"). Migration chạy lại mỗi lần
-- deploy nên ghi đè lặp lại -> ĐÃ XÓA để không bao giờ đụng vào họ tên lãnh đạo.
